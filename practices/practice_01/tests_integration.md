...
- Что проверил студент: проверки `API-1` (413 на 20 001 символе), `REL-1` (timeout 10 с), `SEC-1` (санитизация prompt), `OBS-1` (логи без содержимого) сверены с `CASE.md`; `KeyError` и пустой diff — из рисков P1-01 и `TRAINING_PR.diff`.